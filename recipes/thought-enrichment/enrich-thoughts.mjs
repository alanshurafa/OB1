 ...
 * enrich-thoughts.mjs
 *
 * Retroactively classifies thoughts via OpenRouter or Anthropic API.
...
const STATE_DIR = path.join(PROJECT_ROOT, "data", "import-state");
...
async function main() {
  ...
  while (true) {
    ...
    // Don't offset -- we always query WHERE enriched=false, so rows
    // we just enriched won't appear again. Only offset in dry-run mode.
...
function buildConfig(args, env) {
  ...
  return {
    ...
    // Supabase
    supabaseUrl: env.SUPABASE_URL || `https://${env.SUPABASE_PROJECT_REF}.supabase.co`,
