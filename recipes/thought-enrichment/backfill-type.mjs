...
const env = loadEnv();
const SUPABASE_URL = env.SUPABASE_URL || `https://${env.SUPABASE_PROJECT_REF}.supabase.co`;
...
async function updateBatch(updates) {
  // Updates is array of {id, type}
  // Supabase REST doesn't support bulk PATCH with different values per row,
  // so we do them concurrently in small groups
  const CONCURRENCY = 5;
...
async function main() {
  ...
  const invalidTypeLog = {};   // type value -> count
  const typeDistribution = {}; // new type -> count of updates
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      // Valid type that differs from 'reference' -- queue for update
